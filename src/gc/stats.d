...
import gc = gc.impl.conservative.gc;
import core.memory;
...
// char[40] malloc_stats_file = "malloc.stats";
char[40] malloc_stats_file = "\0";
char[40] collect_stats_file = "collect.stats";
...
private:
    ...
    nothrow void fill_memory_info(MemoryInfo* mem_info)
    {
        // mem_info.overhead += .gc.ConservativeGC.sizeof + gc.pooltable.elements_sizeof +
        ...
        //     }
        // }
        size_t[2] ret = this.gc.fill_memory_info();
        mem_info.used = ret[0];
        mem_info.free = ret[1];
    }

    nothrow cstdio.FILE* start_file(char* filename, char* header)
    {
...
public:
    ...
    {
        import gc.proxy;
        this.gc = cast(.gc.ConservativeGC*) gc.proxy.getInstance;
        // if (malloc_stats_file[0] != '\0') {
        //     this.mallocs_file = this.start_file(
        //             malloc_stats_file.ptr, cast(char *)
        ...
        // }
        // collection
        if (collect_stats_file[0] != '\0') {
            this.collections_file = this.start_file(
                collect_stats_file.ptr, cast(char*)
            /* "Timestamp,Malloc time,*/"Collection time,Stop-the-world time,Used before,Free before,\n".ptr);
            if (this.collections_file !is null)
                this.active = true;
        }
        this.program_start = this.now();
    }

    // nothrow void finalize()
    ~this()
    {
        // if (this.mallocs_file !is null)
    ...
    nothrow void malloc_started(size_t size, uint attr, size_t* ptrmap_bitmask, size_t
            used, size_t free)
    {
        if (!this.active)
            return;
    ...
    nothrow void malloc_finished(void* ptr)
    {
        if (!this.active)
            return;
        auto now = this.now();
    ...
    nothrow void collection_started()
    {
        if (!this.active)
            return;
    ...
    nothrow void collection_finished()
    {
        if (!this.active)
            return;
